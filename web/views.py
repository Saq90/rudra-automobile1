...
def index(request):
    ...
    for i in car:
        carHatchback=Car.objects.filter(body_type="Hatchback")
        carSedan = Car.objects.filter(body_type="Sedan")
        carSuv = Car.objects.filter(body_type="SUV")
        carMuv = Car.objects.filter(body_type="MUV")
        carLuxury = Car.objects.filter(body_type="Luxury")
    # print(car[0])
    n = len(car)
    nSlides = n // 4 + ceil((n / 4) - (n // 4))
    params = {'car': car, 'carHatchback' : carHatchback,'carSedan' : carSedan,'carSuv':carSuv,'carMuv':carMuv,'carLuxury':carLuxury, 'range':range(1,nSlides)}
...
def searchA(request):
    ...
    optradio = request.GET.get('optradio')
    print("Hello ", brand, vehicle, budget, optradio)
    Test = True
    # Each page has 9 requests. That is fixed.
...
def search(request):
    ...
    return render(request, 'web/cars.html',param)
